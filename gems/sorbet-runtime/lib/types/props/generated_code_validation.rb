...
module T::Props
  ...
  module GeneratedCodeValidation
    ...
    def self.validate_deserialize(source)
      ...
      unless name == :__t_props_generated_deserialize_cloned || name == :__t_props_generated_deserialize_frozen
        raise ValidationError.new("Unexpected method_name: #{name}")
      end
      assert_equal(s(:args, s(:arg, :hash), s(:arg, :opts)), args)
    ...
    private_class_method def self.whitelisted_methods_for_deserialize
      @whitelisted_methods_for_deserialize ||= {
        :lvar => %i{freeze dup map transform_values transform_keys each_with_object each nil? []=},
        :const => %i{deserialize from_hash deep_clone_object deep_freeze_object! soft_assert_handler},
        :block => %i{freeze},
      }
    end
  end
